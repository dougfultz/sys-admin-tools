...
hostname = "IP-ADDRESS"
repo = "https://hostupdate.vmware.com/software/VUM/PRODUCTION/main/" \
       "vmw-depot-index.xml"
PerformUpgrade = False
# =Main=====================================================
if __name__ == "__main__":
    # Enable firewall rule
    subprocess.call(["ssh", "-i", sshkey, "-l", username, hostname,
    ...
    subprocess.call(["ssh", "-i", sshkey, "-l", username, hostname,
                     ...
                     "UserVars.SuppressShellWarning long 1"])

    # check for updates
    updateList = subprocess.check_output(["ssh", "-i", sshkey, "-l", username,
                                          hostname,
    ...
    if "update" in updateList.lower():
        ...
        for i in updateList.splitlines():
            if ("status" in i.lower() or "--" in i.lower() or
                    "update" in i.lower()):
                print(i)

        # Perform a dry run
        subprocess.call(["ssh", "-i", sshkey, "-l", username, hostname,
                        "esxcli software vib update --dry-run -d " + repo])

        if PerformUpgrade:
            subprocess.call(["ssh", "-i", sshkey, "-l", username, hostname,
                            "esxcli software vib update -d " + repo])
